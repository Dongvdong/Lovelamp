...
// 自己的MQTT连接ID名称     love+_唯一自定义序列号_+x或y               
//MQTT_MYNAME用于生成唯一通信话题    MQTT_MYNAME+SN(出场芯片唯一ID)用于生成唯一MQTT  ID
...
// 通信话题
#define MY_MQTT_MYTOPIC       "lovelamp/love_00001_x"    
#define YOUR_MQTT_MYTOPIC     "lovelamp/love_00001_y"  

//---------------------------------------------------------------------------------------------
/*
话题格式：
接收   MY_MQTT_MYTOPIC+"/r"     lovelamp/love_00001_x/r
发布   MY_MQTT_MYTOPIC+"/s"     lovelamp/love_00001_x/s
*/
// 自己的话题 
String my_mqtt_topic_s= String()+ MY_MQTT_MYTOPIC+"/s";
String my_mqtt_topic_r= String()+ MY_MQTT_MYTOPIC+"/r";
String your_mqtt_topic_s= String()+ YOUR_MQTT_MYTOPIC+"/s";
String your_mqtt_topic_r= String()+ YOUR_MQTT_MYTOPIC+"/r";
...
struct config_type
{
  ...
  char my_mqtt_topic[100]; //MQTT唯一接收和发布话题  接收/r   发送/s 
  char your_mqtt_topic[100];
};
...
void json_rgb_pub(String R,String G,String B){
   my_mqtt_topic_s= String()+ String(config_wifi.my_mqtt_topic)+"/s";
   ...
   if(client.connected()) {
         ...
         strcpy(  msg_rainc, msg_rain.c_str());
          client.publish(my_mqtt_topic_s.c_str(),msg_rainc); // 强制关闭彩虹动态
           
            strcpy( msg_sc,msg_s.c_str());
          client.publish(my_mqtt_topic_s.c_str(), msg_sc); 
    }    
  ...
  void json_rainbow_pub(String pwm){
     my_mqtt_topic_r= String()+ String(config_wifi.my_mqtt_topic)+"/s";
     String msg_s= "{\"ledmode\":3,\"cc\":"+pwm+"}";
      char msg_c[50];
   strcpy( msg_c,msg_s.c_str());
   if(client.connected()) {
          client.publish(my_mqtt_topic_s.c_str(), msg_c); 
    }    
  }
...
if(sendbegin==0){// 1 没有按键
    ...
    sendtime2++;
    if(sendtime2<=1){
    if(client.connected()){ client.publish(my_mqtt_topic_s.c_str()," {\"ledmode\":5,\"my_hands_on\":1}");   }  
                   }
      }
      else{      
      ...
      two_pwm_go=0;  // 关闭共闪模式
       sendtime2++;
    if(sendtime2<=1){
    if(client.connected()){ client.publish(my_mqtt_topic_s.c_str()," {\"ledmode\":5,\"my_hands_on\":0}");   }  
                   }
        }
...
void saveConfig()
{
 ...
 // Serial.println(config_wifi.mqtt_bool);
  ...
  Serial.println(config_wifi.my_mqtt_topic);
   Serial.println(config_wifi.your_mqtt_topic);
          ...
          strcpy(config_wifi.mqtt_nameid, MQTT_MYNAME);
            strcpy(config_wifi.mqtt_ssid, MQTT_SSID);
              strcpy(config_wifi.mqtt_psw, MQTT_PASSWORD);
                strcpy(config_wifi.my_mqtt_topic, MY_MQTT_MYTOPIC);
                  strcpy(config_wifi.your_mqtt_topic, YOUR_MQTT_MYTOPIC);
...
    if (client.connect(((String)config_wifi.mqtt_nameid+SN).c_str(),config_wifi.mqtt_ssid,config_wifi.mqtt_psw)) {//接入时的用户名，尽量取一个很不常用的用户名
       my_mqtt_topic_s= String()+ String(config_wifi.my_mqtt_topic)+"/s";
       my_mqtt_topic_r= String()+ String(config_wifi.my_mqtt_topic)+"/r";
             client.subscribe(my_mqtt_topic_r.c_str());//接收外来的数据时的intopic          
             client.publish(my_mqtt_topic_s.c_str(),"hello world ");       
       your_mqtt_topic_s= String()+ String(config_wifi.your_mqtt_topic)+"/s";
       your_mqtt_topic_r= String()+ String(config_wifi.your_mqtt_topic)+"/r";
            client.subscribe(your_mqtt_topic_s.c_str());//接收外来的数据时的intopic      
          //  client.publish(your_mqtt_topic_r.c_str(),"hello world ");
                
              Use_Serial.println("Connect succes mqtt!");//重新连接
             
              Use_Serial.println(client.state());//重新连接
              Use_Serial.println("recive topic!");//重新连接
              Use_Serial.println(my_mqtt_topic_r);//重新连接
              Use_Serial.println(your_mqtt_topic_s);//重新连接
              
              Use_Serial.println("send topic!");//重新连接
              Use_Serial.println(my_mqtt_topic_s);//重新连接
              Use_Serial.println(your_mqtt_topic_r);//重新连接
   ...
   if(server.hasArg("mqtt_config")) { // 请求中是否包含有a的参数  
     ...
     if(mqtt_config == "on") { // a=on  
        ...
        if(server.hasArg("mqtt_nameid")) { // 请求中是否包含有a的参数  
         ...
         if(server.hasArg("my_mqtt_topic")) { // 请求中是否包含有a的参数  
          if(server.arg("my_mqtt_topic")!="")
           server.arg("my_mqtt_topic").toCharArray(config_wifi.my_mqtt_topic, 100);    // 从网页得到的 WIFI名
           }

         if(server.hasArg("your_mqtt_topic")) { // 请求中是否包含有a的参数  
          if(server.arg("your_mqtt_topic")!="")
           server.arg("your_mqtt_topic").toCharArray(config_wifi.your_mqtt_topic, 100);    // 从网页得到的 WIFI名
           }
            
         saveConfig();
        
         mqtt_int();
         String msg=String()+"服务器配置成功！请重启生效！<br/> "
                    ...
                    +String(config_wifi.my_mqtt_topic)+"<br/>"
                    +String(config_wifi.your_mqtt_topic)+"<br/>";
